...
/* NEW: pieces to preserve quoted/unquoted segments inside one WORD */
typedef struct s_piece
{
	char			*text;
	int				quoted; /* 1 => no IFS splitting */
	struct s_piece	*next;
}	t_piece;
...
/* NEW: word ctx now also tracks pieces and current quoted-mode */
typedef struct s_word_ctx
{
	t_buf	buf;
	int		buf_quoted; /* NEW: 0/1 indicates current buffer quoted-ness */
	t_piece	*pieces;    /* NEW */
...
typedef struct s_expand_ctx
{
	...
	struct s_word_ctx	*wctx;   /* NEW */
	int					quoted; /* NEW: expansion happens in "quoted" context? */
}	t_expand_ctx;
...
/* NEW: unquoted IFS splitting result with leading/trailing info */
...
//_____________ BASIC_UTILS_HELPER.C
...
t_token		*make_error_token_from_ctx(size_t start,
				const char *msg, t_word_ctx *ctx);
...
int			parse_single_quote(t_buf *buf, const char *line,
				size_t *i, size_t len);
...
//________PARSER_IFS.C
char		**split_by_ifs(const char *s);
void		free_split(char **split);

//_________	PARSER_COMMANDS_HELPER.C
...
//____________EXEC_REDIR_HELPER.C
...
//__________EXEC_COMMAND_HELPER.C
...
void		teardown_resources(int (*pipes)[2], pid_t *pids,
				int n, t_data *data);
...
int			set_env_var_generic(char ***penv, const char *name,
				const char *value);
int			set_env_var_data(t_data *data, const char *name, const char *value);

//_____ENV_UTILS_HELPER.C
...
void		builtin_error(const char *cmd, const char *arg,
				const char *message);
...
//____________PROMPT_USER.C
char		*get_user_host(void);

//___________PROMT_USER_HELPER.C
