...
char *build_prompt(void)
{
	...
	part1 = ft_strjoin("\001" G "\002", cwd);
	free(cwd);
	if (!part1)
		return (ft_strdup("$> "));
	part2 = ft_strjoin(part1, "\001" RST "\002");
	free(part1);
	if (!part2)
		return (ft_strdup("$> "));
	prompt = ft_strjoin(part2, " $> ");
	free(part2);
	if (!prompt)
		return (ft_strdup("$> "));
	return (prompt);
...
int	main(int ac, char **argv, char **envp)
{
	...
	while (data.running == 0)
	{
		...
		if (!prompt)
			prompt = ft_strdup("$> "); /* fallback muy defensivo (idea de copilot, un grande)*/
