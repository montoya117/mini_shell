...
t_ast *parser_commands(t_token **ptokens, t_parser_context *ctx)
{
    ...
    cmd = ast_new_command(NULL, assignments); // luego pondrás argv
    ...
    while (token && (token->type == TOKEN_WORD || token->type == TOKEN_EXPANSION))
    {
        if (token->type == TOKEN_WORD && !token->is_io_number)
        {
        ...
        {
            argv[i] = ft_strdup(token->text);  // expansions NO son io_number
            i++;
        }
        token = token->next;
    }
